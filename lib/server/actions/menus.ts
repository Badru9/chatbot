...
export async function getMenusAction() {
  try {
    ...
    throw new Error(error instanceof Error ? error.message : 'Gagal memuat menu portal.')
  }
}
...
export async function createMenuAction(values: {
  ...
  const { title, description, icon = 'Folder', href, visibleToRoles, order } = values

  if (!title || !description || !href || !Array.isArray(visibleToRoles)) {
    return { error: 'Kolom title, description, href, dan visibleToRoles wajib diisi.' }
  }
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal membuat menu baru.' }
  }
}

export async function updateMenuAction(id: string, values: {
  title?: string
  description?: string
  icon?: string
  href?: string
  ...
  try {
    const updated = await prisma.portalMenu.update({
      ...
      data: {
        ...
        description: values.description !== undefined ? values.description : existing.description,
        icon: values.icon !== undefined ? values.icon : existing.icon,
        href: values.href !== undefined ? values.href : existing.href,
        visibleToRoles: values.visibleToRoles !== undefined ? values.visibleToRoles : existing.visibleToRoles,
        order: values.order !== undefined ? values.order : existing.order,
      },
    })
    return updated
  } catch (error) {
    return { error: error instanceof Error ? error.message : 'Gagal memperbarui menu.' }
  }
}
...
export async function deleteMenuAction(id: string) {
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal menghapus menu.' }
  }
}

export async function reorderMenusAction(reorders: Array<{ id: string; order: number }>) {
  await requireAdmin()
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal memperbarui urutan menu.' }
  }
}
