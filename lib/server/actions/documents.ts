...
import { deleteDocumentChunks, replacePdfChunks } from '@/lib/server/services/database'
...
export async function uploadDocumentAction(formData: FormData) {
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal memproses PDF.' }
  }
}

export async function createManualDatasetAction(input: { name: string; description: string; source?: string }) {
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal menyimpan dataset manual.' }
  }
}
...
export async function deleteDocumentAction(documentId: string) {
  ...
  if (user.role !== 'admin') {
    ...
    if (chunkCount === 0) {
      return { error: 'Forbidden: Dokumen tidak ditemukan atau bukan milik Anda.' }
    }
  }
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : 'Gagal menghapus dokumen.' }
  }
}
